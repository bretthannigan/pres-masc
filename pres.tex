...
\begin{figure}
	...
	\begin{tikzpicture}[ampersand replacement=\&,scale=0.75, every node/.style={scale=0.75}]
		...
		\matrix (m1) [row sep=2.5mm, column sep=5mm]
		{
			...
			\node[dspnodefull]											(m15) {};							\&
			\node[dspnodeopen,dsp/label=right]								(m16) {$y$};							\& \\
			%-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
			\node[coordinate]											(m21) {};							\&
			\node[coordinate]											(m22) {};							\& 
			\node[coordinate]											(m23) {};							\& 
		...
		\draw[dspconn]	(m14) -- (m16);
		\draw[dspline] 	(m15) -- (m25);
		\draw[dspline] 	(m25) -- (m22);
		\draw[dspconn]	(m22) -- (m12);
		\draw[dspconn] 	(m12) -- node[midway,above] {$e$} (m13);
		
		\end{tikzpicture}
	\caption{The sigma delta loop with the quantizer represented as an uncertain gain.}
\end{figure}
\vspace{-0.5cm}
\begin{figure}[h]
	\centering
